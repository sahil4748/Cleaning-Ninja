        ...
        <div className="flex items-center justify-between h-20">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            ...
            <div className="hidden sm:flex flex-col">
              <span className="text-white font-display font-bold text-xl lg:text-2xl leading-tight">
                Cleaning Ninja
              </span>
          ...
          <div className="hidden lg:flex items-center gap-8">
                ...
                <div className="py-2">
                  {SERVICES.map((service) => (
                    <Link
                      ...
                      className="block px-4 py-2 text-sm text-navy hover:bg-accent hover:text-gold transition-colors"
                    >
                      {service.name}
                    </Link>
                  ))}
            ...
            <a
              href={PHONE_LINK}
              className="flex items-center gap-2 text-white hover:text-gold transition-colors font-semibold"
            >
            ...
            {/* CTA Button */}
            <Link href="#quote">
              <Button size="md" className="bg-gold hover:bg-gold/90 text-navy font-bold">
                Get a Quote
              </Button>
            </Link>
          </div>
            ...
            <div className="space-y-2">
              ...
              {SERVICES.map((service) => (
                <Link
                  key={service.id}
                  href={`/services/${service.slug}`}
                  className="block py-2 pl-4 text-white/80 hover:text-gold transition-colors"
                  onClick={() => setMobileMenuOpen(false)}
                >
                  {service.name}
                </Link>
